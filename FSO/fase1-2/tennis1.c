...
float v_pal[10];			/* velocitat de la paleta del programa */

int ipu_pf, ipu_pc;      	/* posicio del la paleta d'usuari */
int ipo_pf[10], ipo_pc[10];      	/* posicio del la paleta de l'ordinador */
float po_pf[10];	/* pos. vertical de la paleta de l'ordinador, en valor real */
...
int inicialitza_joc(void)
{
  ...
  {	
	win_escricar(ipo_pf[i] +i, ipo_pc[i], '1',INVERS);
  }
...
int main(int n_args, const char *ll_args[])
{
 ...
 /* if ((n_args != 2) && (n_args !=3))
  ...
  do
  {
    pthread_create(&tid[i+1], NULL, mou_paleta_ordinador,(i-1));
    i++;
  }while(i<=num_oponents);
